...
class RedisClientLib
{
    ...
    public function GetSizeOfList($key){
        return $this->cmd("LLEN", $key)->get();
    }
    ...
    public function Enqueue($queueName, $value, $expireInSeconds = null)
    {
        if (is_null($expireInSeconds)) {
            $this->cmd("LPUSH", $queueName, $value)->set();
        }else{
            $this->cmd("LPUSH", $queueName, $value)->cmd("EXPIRE", $queueName, $expireInSeconds)->set();
        }
    }
